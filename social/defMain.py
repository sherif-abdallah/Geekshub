...
def check_if_company_account_didnt_send_to_user(request):
    company_account_id = User.objects.get(username="Geekshub").id
    messages_inbox = Messages_Inbox.objects.filter(to_user_id=request.user.id, from_user=company_account_id)
    if len(messages_inbox) == 0:
        ...
        Messages_Inbox.objects.create(to_user_id=request.user.id, from_user_id=company_account_id, date_utc=datetime.now(timezone.utc).strftime("%Y-%m-%d %I:%M:%S %p"), msg=msg)
