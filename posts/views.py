...
from social.defMain import cheek_user_picture, paginate
...
def Post_views(request, id):
    ...
    cheek_user_fav = Post_Favorite.objects.filter(user_id=request.user.id, post_id=post.id) # Cheek If user has fav the post
    ...
    if request.method == "POST":
        ...
        # ------------------------------------------------------------------------------------------------------------------------------
        # If Add to Favorite Form Is Submited
        if request.POST.get('favorite_valid') == "True":
            cheek_user_fav = Post_Favorite.objects.filter(user_id=request.user.id, post_id=post.id) # Cheek If user has fav the post
            cheek_user_fav = len(cheek_user_fav)

            if cheek_user_fav == 0:
                Post_Favorite.objects.create(user_id=request.user.id, post_id=post.id)
            # Like
            else:
                Post_Favorite.objects.filter(user_id=request.user.id, post_id=post.id).delete()
        # ------------------------------------------------------------------------------------------------------------------------------
