...
from .models import *
from social.defMain import cheek_user_picture, paginate, Reverse
...
def Notification_Json_Api_Views(request):
    notification_bar = Notifications.objects.filter(user=request.user.id, is_seen=False)
    notification_bar_len = len(notification_bar)

    return JsonResponse({"notification_bar_len":notification_bar_len})